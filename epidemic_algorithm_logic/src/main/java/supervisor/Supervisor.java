...
public class Supervisor {
    ...
    private Map<String, Integer> addressToNodeId; // Map<"tcpAddress", nodeId> for discovered nodes (using TCP address
                                                  // as key)
    private Map<Integer, Address> discoveredNodeAddresses; // Map<nodeId, UDP Address> for discovered nodes (for
                                                           // StartRoundMsg)
    private Map<Integer, Address> discoveredNodeTcpAddresses; // Map<nodeId, TCP Address> for discovered nodes (for
                                                              // StartNodeMsg/KillNodeMsg)
    ...
    private static final int SEARCHING_TIMEOUT_MS = 20000; // 20 seconds
    ...
    /**
     * Get TCP communication for nodes (distributed mode) - may be null in local
     * mode
     */
    ...
    private void startLocalNetwork(StartMsg startMessage) {
        ...
        // Initialize TCP server for node communication (needed for StartNodeMsg and
        // InfectionUpdateMsg/RemotionUpdateMsg)
        ...
        // Supervisor will discover nodes via HelloMsg, ignoring the addresses it
        // created
        // The created addresses are only used to create the DistributedNodeStub
        // instances
        isSearching = true;
    ...
    /**
     * Handle HelloMsg: Map TCP address to Node ID (incremental) and store both TCP
     * and UDP addresses
     */
    ...
    /**
     * Resend StartNodeMsg to a specific node (used for recovery/late joiners)
     */

    /**
     * Finish SEARCHING mode: Calculate topology and send StartNodeMsg to all
     * discovered nodes
     */
    private void finishSearchingAndSendStartNodeMsg(StartMsg startMessage) {
        ...
        try {
            ...
            String protocolRaw = startMessage.getProtocol(); // e.g. "anti-entropy"
            String mode = startMessage.getMode();

            // Sanitize protocol
            // 1. Map generic "gossip" to "blind_coin"
            // 2. Normalize "anti-entropy" to "anti_entropy" (hyphen to underscore)
            String protocol = "gossip".equalsIgnoreCase(protocolRaw) ? "blind_coin" : protocolRaw;
            if (protocol != null) {
                protocol = protocol.replace('-', '_');
            }

            // Get NetworkStructureManager - use existing one in local mode, create new in
            // distributed mode
            // Get NetworkStructureManager - use existing one in local mode, create new in
            // distributed mode
            ...
            if (system != null && system.getNetworkStructureManager() != null) {
                ...
                // WAIT: Supervisor doesn't have access to NetworkStructureManager constructor
                // if it's package private?
                // Checking imports... it is imported as
                // supervisor.network_emulation.neighbors_and_subject.NetworkStructureManager
            ...
            // HelloMsg contains both TCP and UDP addresses - use UDP addresses for
            ...
            for (Map.Entry<Integer, Address> entry : discoveredNodeTcpAddresses.entrySet()) {
                ...
                // Nodes listen for StartNodeMsg via TCP on the same port as UDP (TCP and UDP
                // can share ports)
                String encodedMsg = startNodeMsg.encode();

                // Send via TCP using the TCP address from HelloMsg (nodes are listening for
                // StartNodeMsg on TCP)
                nodeTcpCommunication.sendMessage(nodeTcpAddress, encodedMsg);

                System.out.println(
                        "[Supervisor] Sent StartNodeMsg to node " + nodeId + " at " + nodeTcpAddressStr + " via TCP");
            }
    ...
    private void sendStartRoundPeriodically() {
        try {
            ...
            while (isNetworkRunning && (system != null || isDistributedMode)) {
                ...
                // Use discovered addresses (filled in finishSearchingAndSendStartNodeMsg for
                // both modes)
                if (discoveredNodeAddresses != null && !discoveredNodeAddresses.isEmpty()) {
                    nodeAddresses = discoveredNodeAddresses;
                } else {
                    // Fallback: Use NetworkEmulator addresses in local mode (if discoveredAddresses
                    // not yet filled)
    ...
    public void disableExternalUi() {
        if (externalUiAvailable) {
            System.out.println(
                    "External UI not available. Disabling external UI message sending. (Local GUI still active)");
            externalUiAvailable = false;
        }
    }
    ...
    public void endNetwork(EndMsg endMessage) {
        ...
        if (isDistributedMode && addressToNodeId != null && !addressToNodeId.isEmpty()
            ...
            try {
                ...
                // Send KillNodeMsg to all discovered nodes via TCP (using TCP addresses from
                // HelloMsg)
     ...
     * 
     * @param supervisorPort Port number for supervisor to listen on (UDP for nodes,
     *                       TCP for UI on same port)
     */
    public void initialize(int supervisorPort) {
        ...
        // Initialize WebSocket server on port 8087
