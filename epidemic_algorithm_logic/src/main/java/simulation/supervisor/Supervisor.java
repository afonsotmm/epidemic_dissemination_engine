...
public class Supervisor {
    ...
    // Storage for remotion information: Map<nodeId, Set<MessageId>> (nodes that have removed messages)
    ...
    // Map to store which nodes are sources and their subjects (for GUI, without direct node access)
    ...
    // Track last round with new infections (for stopping condition: 2 rounds without infections)
     ...
     * @param nodeMode Mode of operation: PULL, PUSH, PUSHPULL, or BLIND_COIN_PUSH
     */
     ...
     * @param supervisorPort Port for supervisor to listen on
     * @param nodeMode Mode of operation: PULL, PUSH, PUSHPULL, or BLIND_COIN_PUSH
     * @param blindCoinK Probability parameter for Blind Coin (1/k chance to stop spreading)
     */
     ...
     * Uses different localhost IPs (127.0.0.1, 127.0.0.2, ..., 127.0.X.0) with same port
     */
    ...
    private List<MessageTopic> generateSubscribedTopics(Map<Integer, String> subjectsForNodes) {
        ...
        if (subjectsForNodes != null) {
            for (Map.Entry<Integer, String> entry : subjectsForNodes.entrySet()) {
                ...
                if (subject != null) {
                    // Create MessageTopic with subject and sourceId (timestamp not needed for subscription)
     ...
     * @param subjectsForNodes Map of nodeId -> subject (null if node is not a source)
     */
     ...
     * Format: node_to_supervisor;infection_update;subject;timestamp;sourceId;updated_node_id;infecting_node_id
    ...
    private void processInfectionUpdateMessage(String encodedMessage) {
        try {
            ...
            epidemic_core.message.node_to_supervisor.infection_update.InfectionUpdateMsg infectionUpdateMsg = 
                epidemic_core.message.node_to_supervisor.infection_update.InfectionUpdateMsg.decodeMessage(encodedMessage);
     ...
     * Format: node_to_supervisor;remotion_update;subject;timestamp;sourceId;updated_node_id
     * @param encodedMessage The encoded message string
     */
    private void processRemotionUpdateMessage(String encodedMessage) {
        try {
            ...
            epidemic_core.message.node_to_supervisor.remotion_update.RemotionUpdateMsg remotionUpdateMsg = 
                epidemic_core.message.node_to_supervisor.remotion_update.RemotionUpdateMsg.decodeMessage(encodedMessage);
     ...
     * 
     * For Gossip nodes: only count messages that are NOT removed (can still be spread)
    ...
    private boolean hasConverged() {
        ...
        // Converged if: all nodes have spreadable messages OR no nodes have spreadable messages
        boolean converged = nodesWithSpreadableMessages == numberOfNodes || nodesWithoutSpreadableMessages == numberOfNodes;
    ...
    public void triggerPullRound() {
        ...
        epidemic_core.message.supervisor_to_node.start_round.StartRoundMsg startRoundMsg = 
            new epidemic_core.message.supervisor_to_node.start_round.StartRoundMsg(
        ...
        // Schedule convergence check after 80% of round interval (to give time for messages to be processed)
    ...
    private void checkConvergenceAndStop() {
        if (hasConverged()) {
            // Count infected nodes (with spreadable messages) and susceptible nodes (without spreadable messages)
            int infectedNodes = 0;
    ...
    public void generateInfectionChart(String subject, Integer sourceId) {
        ...
        // First pass: find first infection round for each node (filtered by subject and sourceId)
        for (Map.Entry<Integer, List<InfectionRecord>> entry : infectionHistory.entrySet()) {
            int nodeId = entry.getKey();
        ...
        for (int round = 1; round <= maxRound; round++) {
            ...
            // Stop adding data when all nodes are infected (chart shows infections, but auto-update stops when all are infected and/or removed)
        ...
        System.out.println("Chart displayed/updated for " + filterInfo + ". Total infections: " + totalInfected + "/" + numberOfNodes + " (Round " + currentRound + ")");
    ...
    /**
     * Generate and display a chart showing infections over rounds (all subjects and sources)
     */
    ...
    /**
     * Generate and display a chart showing infections over rounds for a specific subject
